...
class TestAnthropicOpenAIBridge:
    def test_init_with_default_config(self):
        ...
        with patch.object(
            ConfigManager, "__init__", return_value=None
        ) as mock_config_init:
            mock_config = Mock()
            mock_config_init.return_value = None

            with patch(
                "anthropic_openai_bridge.bridge.ConfigManager", return_value=mock_config
            ):
                with patch("anthropic_openai_bridge.bridge.OpenAIClientWrapper"), patch(
                    "anthropic_openai_bridge.bridge.RequestConverter"
    ...
    def test_init_with_custom_openai_api_key(self):
        ...
        with patch(
            "anthropic_openai_bridge.bridge.ConfigManager"
        ) as mock_config_class, patch(
            "anthropic_openai_bridge.bridge.OpenAIClientWrapper"
        ), patch(
            "anthropic_openai_bridge.bridge.RequestConverter"
        ), patch(
            "anthropic_openai_bridge.bridge.ResponseConverter"
        ):
            ...
            mock_config_class.assert_called_once_with(
                openai_api_key="custom_key", openai_base_url=None, httpx_client=None
            )
            assert bridge.config == mock_config
    ...
    def test_send_message_flow(self):
        ...
        with patch("anthropic_openai_bridge.bridge.ConfigManager"), patch(
            "anthropic_openai_bridge.bridge.OpenAIClientWrapper"
        ) as mock_client_class, patch(
            "anthropic_openai_bridge.bridge.RequestConverter"
        ) as mock_req_converter_class, patch(
            "anthropic_openai_bridge.bridge.ResponseConverter"
        ) as mock_resp_converter_class:
            ...
            # Create a proper Anthropic Message object for the mock response
            anthropic_response = anthropic.types.Message(
                id="msg_12345678",
                type="message",
                role="assistant",
                content=[anthropic.types.TextBlock(type="text", text="response")],
                model="test",
            ...
            # Verify result is an Anthropic Message object
